...
def training_model(inp_train: pd.DataFrame, out_train : pd.Series,
...
def apply_minist():
    ...
    my_model = training_model(inp_train, out_train, inp_test, out_test, 5)

    # checking overall accuracy
    inp_test1 = inp_test.to_numpy()
    inp_test1 = np.delete(inp_test1, [0], 1)
    inp_test1 = np.reshape(inp_test1, (1217, 28, 28))
    loss, accuracy = my_model.evaluate(inp_test1, out_test, verbose=2)
    ...
    for designer in range(5):
        ...
        loss, accuracy = my_model.evaluate(inp_test3, out_test3, verbose=2)
        print('\nAccuracy by designer', designer, ":", accuracy * 100, " length is", length)
    ...
    # making a model for label "designer"
    my_model = training_model(inp_train, out_train, inp_test, out_test, 21)
    ...
    for year in range(21):
        ...
        inp_test3 = np.reshape(inp_test3, (inp_test3.shape[0], 28, 28))
        loss, accuracy = my_model.evaluate(inp_test3, out_test3, verbose=2)
        print('\nAccuracy by year', year, ":", accuracy * 100, " length is", length)
